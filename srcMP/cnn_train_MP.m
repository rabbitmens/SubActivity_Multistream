function [net, info] = cnn_train_MP(net, imdb, getBatch, varargin)
...
function err = error_mergeclassMP(opts,labels,res)
predictions = gather(res(end-1).x) ;
pred1 = predictions(:,:,1:67,:);
pred2 = predictions(:,:,68:end,:);
% pred2 = predictions(:,:,11:22,:);
[~,pred1] = sort(pred1, 3, 'descend') ; % 
% [~,pred2] = sort(pred2, 3, 'descend') ; % 
...
error3 = 0;
for bind = 1 : size(objlabel,1)
    place = find(i==bind);
    labbat = j(place);
    curpred = spltruesort(:,bind);
    a = ismember(labbat,curpred);
...
function err = error_mergeclass(opts,labels,res)
predictions = gather(res(end-1).x) ;
pred1 = predictions(:,:,1:10,:);
pred2 = predictions(:,:,11:22,:);
[~,pred1] = sort(pred1, 3, 'descend') ; % 
[~,pred2] = sort(pred2, 3, 'descend') ; % 
[~,predictions] = sort(predictions, 3, 'descend') ;

labels = reshape(labels,1,1,2,[]) ;
...
% 
% % skip null labels
mass = single( (labels(:,:,1,:) > 0)&(labels(:,:,2,:) > 0)) ;
% if size(labels,3) == 2
...
error1 = ~bsxfun(@eq, pred1(:,:,1:5,:), labels(:,:,1,:)) ;
error2 = ~bsxfun(@eq, pred2(:,:,1:5,:), labels(:,:,2,:)) ;
error = bsxfun(@plus,error1,error2)/2 ;
