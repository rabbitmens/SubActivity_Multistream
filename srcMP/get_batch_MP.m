function imo = get_batch_MP(images, varargin)
...
% imo = zeros(opts.imageSize(1), opts.imageSize(2), 3 * opts.sequencenum, ...
%             numel(images)*opts.numAugments, 'single') ;
imo = zeros(opts.imageSize(1), opts.imageSize(2), 3 * opts.sequencenum, 'single') ;
...
for i=1:numel(images)
  ...
  % crop middle
  small = min(opts.border);
  tosize = uint16(opts.imageSize + small);
  simt =uint16(size(imt));
  simt = simt(1:2);
  dif = simt-tosize;
  dif = uint16(dif/2);
  dif = dif+1;
